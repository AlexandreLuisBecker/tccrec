...
from script.database import connection, cursor
    ...
    def insert_values_in_dabatase(self):
        ...
        try:
            ...
            cursor.execute(
                """
                UPDATE users
                SET nome = ?, cpf = ?, cargo = ?, email = ?
                WHERE id = ? """,(str(self.txt_login), str(self.txt_cpf), str(self.txt_cargo), str(self.txt_email),int(ide)))
        except ValueError:
